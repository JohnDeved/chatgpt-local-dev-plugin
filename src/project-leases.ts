...
const scopeSchema = z.object({ path: z.string(), device: z.string(), inode: z.string(), head: z.string().nullable() }).strict();
const leaseSchema = z.object({ generation: uuid, owner: ownerSchema, scope: scopeSchema, mode: z.enum(["read", "write"]), ttl: z.number(), expires: z.number(), pending: z.boolean(), reads: z.number().int().nonnegative(), writes: z.number().int().nonnegative(), background: z.array(z.number().int().positive()) }).strict();
...
const idle = (lease: Lease): boolean => lease.reads === 0 && lease.writes === 0 && lease.background.length === 0;
...
export async function projectScope(path: string): Promise<Scope> {
  ...
  return { path: canonical, device: String(info.dev), inode: String(info.ino), head };
}
function matches(actual: Scope, expected: Scope, head: boolean): void {
  if (actual.path !== expected.path || actual.device !== expected.device || actual.inode !== expected.inode) throw new LeaseError("PROJECT_REPLACED", { path: expected.path });
...
export class ProjectLeases {
  ...
  private async reconcile(state: State): Promise<void> {
    ...
    state.leases = state.leases.filter(lease => {
      ...
      const uncertainWrite = lease.writes > 0 || (lease.pending && lease.mode === "write");
      const reclaim = lease.background.length === 0 && !uncertainWrite && (!live || (!lease.pending && lease.reads === 0 && lease.expires <= Date.now()));
  ...
  async reserve(session: string, path: string, options: OpenLeaseOptions, previous?: string): Promise<Lease> {
    ...
    return await this.store.transaction(async state => {
      ...
      const old = previous ? state.leases.find(value => value.generation === previous && same(value.owner, owner)) : undefined;
      if (old && !idle(old)) throw new LeaseError("PROJECT_HAS_ACTIVE_WORK");
      const blockers = state.leases.filter(value => value !== old && overlaps(value.scope.path, scope.path) && (value.mode === "write" || options.mode === "write"));
      ...
      const lease: Lease = { generation: randomUUID(), owner, scope, mode: options.mode, ttl, expires: Date.now() + ttl, pending: true, reads: 0, writes: 0, background: [] };
  ...
  async commit(session: string, generation: string, previous?: string, handoffId?: string): Promise<{ lease: Lease; previousRelease: { released: false; handoffId: string } | null }> {
    const owner = await this.owner(session);
    return await this.store.transaction(async state => {
      await this.reconcile(state);
      const next = this.owned(state, generation, owner), scope = await projectScope(next.scope.path);
      ...
      if (handoffId) {
        ...
        if (same(receipt.from, owner)) throw new LeaseError("HANDOFF_NOT_INDEPENDENT");
        matches(scope, receipt.scope, true);
        if (state.leases.some(value => same(value.owner, receipt.from) && overlaps(value.scope.path, scope.path))) throw new LeaseError("SOURCE_REACQUIRED");
      }
  ...
  async abort(session: string, generation: string): Promise<void> {
    const owner = await this.owner(session);
  ...
  async access(session: string, generation: string, write: boolean): Promise<void> {
    const owner = await this.owner(session);
    await this.store.transaction(async state => {
      ...
      if (write) lease.writes++; else lease.reads++;
      lease.expires = Date.now() + lease.ttl;
    });
  }
  async complete(session: string, generation: string, write: boolean, background: number[] = []): Promise<void> {
    const owner = await this.owner(session);
    await this.store.transaction(async state => {
      const lease = this.owned(state, generation, owner), key = write ? "writes" : "reads";
      if (lease[key] < 1) throw new LeaseError("LEASE_OPERATION_MISMATCH");
      lease[key]--; lease.background = background; lease.expires = Date.now() + lease.ttl;
    });
  }
  async release(session: string, generation: string): Promise<{ released: false; relinquished: true; handoffId: string }> {
    const owner = await this.owner(session);
    return await this.store.transaction(async state => {
      const lease = this.owned(state, generation, owner), scope = await projectScope(lease.scope.path);
  ...
  async forceRelease(session: string, path: string, generation: string, reason: string): Promise<{ relinquished: true; released: false }> {
    ...
    return await this.store.transaction(async state => {
      ...
      if (lease.writes || (lease.pending && lease.mode === "write") || lease.background.length || (!dead && (lease.pending || lease.reads || lease.expires > Date.now()))) throw new LeaseError("LEASE_NOT_RECLAIMABLE", { owner: lease.owner, generation, reason: "Live work is not evicted; use the owning session's cooperative lifecycle." });
  ...
  async handoff(id: string): Promise<{ released: boolean; receipt: ReleaseReceipt; reason: string | null }> {
    return await this.store.transaction(async state => {
      ...
      const reason = !receipt.to ? "INDEPENDENT_ACQUISITION_PENDING" : state.leases.some(value => same(value.owner, receipt.from) && overlaps(value.scope.path, receipt.scope.path)) ? "SOURCE_REACQUIRED" : null;
      return { released: reason === null, receipt: structuredClone(receipt), reason };
    });
  }
  async close(): Promise<void> { if (this.liveHandle) await (await this.liveHandle).close(); }
}
