...
const startSchema = z.object({ goal, title: z.string().trim().min(1).max(120).optional(), plan: summary.optional() }).strict();
const updateSchema = z.object({ runId, summary, kind: z.enum(["plan", "progress", "decision"]).default("progress"),
  ...
  todos: z.array(z.object({ id: z.string().regex(/^[a-zA-Z0-9_-]{1,80}$/u), title: z.string().trim().min(1).max(500).optional(), steeringId: z.string().min(1).max(80).optional(), status: z.enum(["queued", "in_progress", "paused", "completed", "cancelled"]), note: z.string().trim().min(1).max(2000).optional() }).strict()).max(100).default([]) }).strict();
const finishSchema = z.object({ runId, outcome: z.enum(["completed", "failed", "cancelled"]), summary }).strict();

export function runTools(tracker: RunTracker, backgroundCount: (id: string) => number): RegistryEntry[] {
  const definitions = [
    { name: "run.start", title: "Start worker run", schema: startSchema,
      description: "Call once before local work for each user request. Publish the user's goal and a brief plan. An observed run is promoted, not duplicated. Keep the returned runId. Do not invent a goal or claim work has finished." },
    { name: "run.update", title: "Update worker run", schema: updateSchema,
      description: "Publish a concise user-facing plan, progress, or decision summary. Not private chain-of-thought. Acknowledge local steering IDs returned in tool responses before further work, and explain the next step. Publish todos with stable IDs and titles before substantive work (the dispatcher enforces this); link a todo to a direction with steeringId when appropriate; update their statuses and notes without forgetting paused work when steering arrives. Use steeringTasks to track the work separately from delivery: queued, in_progress, paused, completed, or cancelled, with a truthful outcome note. Completed means implemented/verified as reported, not merely acknowledged. Use the runId returned by run.start." },
    { name: "run.finish", title: "Finish worker run", schema: finishSchema,
      description: "Call before your final answer to explicitly end the user's local work with completed, failed, or cancelled and a factual summary. Cannot report completed with active tool operations, unacknowledged steering, or unresolved steering tasks. Failed/cancelled outcomes retain unfinished tasks rather than pretending they were implemented. This does not stop background services." },
  ] as const;
    ...
    call: (input, context) => {
      ...
      try {
        ...
        if (definition.name === "run.start") {
          ...
          run = tracker.start(owner, value.goal, value.title);
          ...
          run = tracker.update(owner, value.runId, value.summary, value.kind, value.acknowledgedSteeringIds, value.goal, value.steeringTasks, value.todos);
        } else {
          const value = finishSchema.parse(input);
          run = tracker.finish(owner, value.runId, value.outcome, value.summary, backgroundCount(value.runId));
        }
