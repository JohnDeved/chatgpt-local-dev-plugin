...
const SERVER_INSTRUCTIONS = [
  ...
  "Prefer an available downstream semantic code tool for symbol-aware navigation/refactoring. For ordinary text files, use native project.write to create and project.read plus project.edit to modify; these do not depend on a downstream server.",
  "Pass raw text/newlines to editing tools, not Python/Node code strings, shell heredocs, regex replacements or encoded wrappers. The tools preserve bytes and formatting; run the project's formatter separately when needed. project.read returns the current sha256; provide it as expectedSha256. A changed hash or ambiguous/missing oldText means reread and refine the edit, never blindly overwrite.",
  "If this connection lacks project.write/project.edit or configured downstream tools, report the tool-discovery/activation gap instead of claiming the editor failed. Existing connections require the updated runtime and a tool refresh; do not restart the tunnel or change approval settings without the user's direction. Use dev.run for builds/tests/generators, not routine text edits when the native tools are available.",
